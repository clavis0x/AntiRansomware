...
CGdipButton::CGdipButton()
{
	...
	m_bHaveSthImage = FALSE;
	m_bHaveAltImage = FALSE;
	m_bHaveAlhImage = FALSE;
...
CGdipButton::~CGdipButton()
{
	...
	if(m_pSthImage) delete m_pSthImage;
	if(m_pAltImage) delete m_pAltImage;
	if(m_pAlhImage) delete m_pAlhImage;
...
BOOL CGdipButton::LoadSthImage(UINT id, LPCTSTR pType)
{
	m_bHaveSthImage = TRUE;
	m_pSthImage = new CGdiPlusBitmapResource;
	return m_pSthImage->Load(id, pType);
}

BOOL CGdipButton::LoadAlhImage(UINT id, LPCTSTR pType)
{
	m_bHaveAlhImage = TRUE;
	m_pAlhImage = new CGdiPlusBitmapResource;
	return (m_pAlhImage->Load(id, pType));
}
		...
		if (m_dcStd.m_hDC == NULL)
		{
			...
			if(m_dcStdH.m_hDC == NULL)
			{
				...
				if(!m_bHaveSthImage){
					...
					*/
					ColorMatrix HotMat = { 1.00f, 0.00f, 0.00f, 0.00f, 0.00f,
					...
					graphics.DrawImage(*m_pSthImage, 0, 0);
				}
			...
			if(m_dcAltH.m_hDC == NULL)
			{
				...
				if (!m_bHaveAlhImage) {
					...
					*/
					...
					graphics.DrawImage(*m_pAlhImage, 0, 0);
				}
...
void CGdipButton::SetToggle(BOOL bSet)
{
	if (bSet == TRUE) {
		m_bIsToggled = TRUE;
		m_nCurType = ALT_TYPE;
